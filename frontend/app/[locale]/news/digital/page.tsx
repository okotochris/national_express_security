...
const translations: Translations = {
  ...
  fr: {
    ...
    heroBtn: "Inscrivez-vous pour les mises à jour →",
    ...
    <>
      ...
      <section className="relative w-full bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-800 text-white py-28 overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <Image src="/presenting.jpg" alt="Digital network background" fill className="object-cover" />
        </div>
      ...
      <section className="py-20 px-6 bg-gradient-to-b from-white to-blue-50">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            ...
            <Link href="/signup" className="inline-block bg-emerald-600 text-white px-6 py-3 rounded-md font-semibold hover:bg-emerald-700 transition">
              {t.signupBtn}
            </Link>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="relative w-full h-[350px] md:h-[450px]">
            <Image src="/image3.jpg" alt="Digital shipment tracking dashboard" fill className="object-cover rounded-2xl shadow-2xl" priority />
          </motion.div>
        </div>
      </section>
